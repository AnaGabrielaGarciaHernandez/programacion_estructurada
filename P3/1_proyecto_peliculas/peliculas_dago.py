...
def buscarPeliculas():
  ...
  if conexionBD!=None:
    ...
    #coma al final cuando es un parametro
    val=(nombre,)
    cursor.execute(sql,val)
    #existe fetchone donde solo arroja el primero encontrado, usar solo con primary key
    registros=cursor.fetchall()
    if registros:
     print(f"\n\tMostrar la Pelicula")
     print(f"{'ID':<10}{'Nombre':<15}{'Categoria':<15}{'Clasificación':<15}{'Genero':<15}{'Idioma':<15}")
  ...
  if conexionBD!=None:
    ...
    if registros:
      ...
      for fila in registros:
        print(f"{fila[0]:<10}{fila[1]:<15}{fila[2]:<15}{fila[3]:<15}{fila[4]:<15}{fila[5]:<15}")
        print(f"-"*80)  
      resp=input(f"¿Deseas borrar la peliculas de {nombre}? S/N ").lower().strip()
      if resp=="si":
